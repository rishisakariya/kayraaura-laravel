...
class OrderShipmentLifecycleService
{
    ...
    public function applyForwardStatus(OrderShipment $shipment, string $normalizedStatus): void
    {
        ...
        if ($dirty) {
            ...
            Log::channel('thirdparty')->info('Shipment lifecycle: forward delivery applied to order', [
                'order_id' => $order->id,
    ...
    public function completeReturnIfReceived(OrderShipment $shipment, string $normalizedStatus): void
    {
        ...
        try {
            DB::transaction(function () use ($order) {
                ...
                Log::channel('thirdparty')->info('Return flow: return received at warehouse', [
                    'order_id' => $order->id,
                    'order_number' => $order->order_number,
                    'shipment_id' => $shipment->id,
            ...
            Log::channel('thirdparty')->error('Return completion failed after reverse delivery', [
                'order_id' => $order->id,
                'shipment_id' => $shipment->id,
                'error' => $e->getMessage(),
            ]);
